-- GHMD Sales Platform — Decision Log historical backfill
-- Source: frozen Google Doc archive "GHMD Sales Platform — Decision Log"
--   (Doc ID 1SEZcEVzTw_DKHUjaij6jdK7BEMYDFWm0iB5BMTcdcOY)
-- The Google Doc is NOT deleted — it becomes a frozen archive. This script
-- mirrors its 16 historical entries into ops.decision_log. The Capture
-- Taxonomy v1 seed (scripts/seed_capture_taxonomy.sql) is loaded FIRST,
-- before this backfill runs.
--
-- Status mapping notes (enum is ADOPTED/LOCKED/PLANNED/REJECTED/CONFIRMED/SUPERSEDED/OPEN):
--   "PERMANENTLY REMOVED"  -> REJECTED   (Whisper)
--   "PARTIALLY SUPERSEDED" -> SUPERSEDED (Spoke Candidate Data Source)
--   Status is taken from the entry's STATUS line, not the header label
--   (e.g. "Outcome Metrics — LOCKED" header but STATUS line = ADOPTED).
-- legal_flag = true where the entry carries an explicit legal note/flag.
-- source_session = null for all historical entries (pre-dates session tagging).

-- ---------------------------------------------------------------------------
-- PASS 1 — insert historical entries
-- ---------------------------------------------------------------------------
...
('2026-06-27', 'sales',
 'Operator Score Architecture — LOCKED',
 $DEC$Two-tier scoring architecture locked for operator qualification. Structured triage output (proceed / conditional / pass), not a weighted numeric score at this stage.
...
Spend caps — hard caps AND alerts on every paid surface before any prospect-facing AI route goes live: Anthropic (Claude API), OpenAI (if added), Netlify credit usage (Pro is credit-metered — documented surprise-overage risk). Opens Open Item #20.$DEC$,
 $RSN$Cross-reference: pattern adopted in the NIP build (NIP Decision Log 2026-06-26). The Sales Platform carries the fuller business-logic surface — the NIP required only the AI-endpoint subset — so the full architecture is planned independently here. When to build: a dedicated security sprint. Prerequisite: at least one AI route (Claude API extraction) live in production, because Layer 2 rules require real usage patterns to calibrate quotas. Build before any prospect-facing AI feature is publicly accessible. Status: PLANNED — do not build now.$RSN$,
 'PLANNED', false, null),

-- [2026-06-26] Deepgram Nova-3 — REJECTED (permanent)
('2026-06-26', 'sales',
 'Deepgram Nova-3 — REJECTED (permanent)',
 $DEC$Deepgram Nova-3 permanently rejected as transcription engine. Do not revisit.$DEC$,
 $RSN$7.32% MER vs AssemblyAI's 4.97% (32% higher error rate on medical terminology per independent Hamming.ai benchmarks). No dedicated Medical Mode endpoint. Regex-based PII redaction. Speed advantage irrelevant for the batch use case.$RSN$,
 'REJECTED', false, null),

-- [2026-06-26] Fireflies.ai — REJECTED (permanent)
('2026-06-26', 'sales',
 'Fireflies.ai — REJECTED (permanent)',
 $DEC$Fireflies.ai permanently rejected as transcription/capture solution. Do not revisit.$DEC$,
 $RSN$End-user product, not infrastructure. Opaque credit system. HIPAA compliance only on the Enterprise tier. 90–95% accuracy with known degradation. Not suitable as pipeline backbone.$RSN$,
 'REJECTED', false, null),

-- [2026-06-26] Plaud — REJECTED (permanent)
('2026-06-26', 'sales',
 'Plaud — REJECTED (permanent)',
 $DEC$Plaud permanently rejected as capture solution. Do not revisit.$DEC$,
 $RSN$Personal productivity device, not infrastructure. Hardware dependency. Not scalable to Leif or a future team.$RSN$,
 'REJECTED', false, null),

-- [2026-06-26] Whisper — PERMANENTLY REMOVED from Architecture  (status mapped REJECTED)
('2026-06-26', 'sales',
 'Whisper — PERMANENTLY REMOVED from Architecture',
 $DEC$Whisper permanently removed from the GHMD Sales Platform architecture. Do not reinstate.$DEC$,
 $RSN$Replaced by AssemblyAI Universal-3 Pro + Medical Mode as transcription engine. AssemblyAI's 4.97% MER on medical terminology is superior to Whisper's performance, it has native Recall.ai integration, and it signs a BAA for PHI compliance.$RSN$,
 'REJECTED', false, null),

-- [2026-06-26] AssemblyAI Universal-3 Pro + Medical Mode — SELECTED (transcription engine)
('2026-06-26', 'sales',
 'AssemblyAI Universal-3 Pro + Medical Mode — SELECTED (transcription engine)',
 $DEC$AssemblyAI Universal-3 Pro with Medical Mode selected as the transcription engine for all intro calls. Locked — do not revisit.$DEC$,
 $RSN$4.97% MER on medical terminology vs Deepgram's 7.32% (32% lower, independent Hamming.ai benchmarks). Native Recall.ai integration (single API parameter at bot creation). Signs a BAA for PHI compliance. All-in cost ~$0.87/hour (~$0.65 per 45-min intro call).$RSN$,
 'LOCKED', false, null),

-- [2026-06-26] Recall.ai Meeting Bot API — SELECTED (call capture)
('2026-06-26', 'sales',
 'Recall.ai Meeting Bot API — SELECTED (call capture)',
 $DEC$Recall.ai Meeting Bot API selected for call capture and a transcript-ready webhook. Locked — do not revisit.$DEC$,
 $RSN$Native integration with AssemblyAI. Bot named "GHMD Call Notes" — disclosed at the open of every intro call (standing consent rule). Fallback: Recall.ai Desktop Recording SDK (same pipeline, no bot in room, no architecture change).$RSN$,
 'LOCKED', false, null),

-- [2026-06-26] Summary Layer — REJECTED
('2026-06-26', 'sales',
 'Summary Layer — REJECTED',
 $DEC$Summary layer rejected as redundant in the two-layer capture architecture. Not part of the architecture.$DEC$,
 $RSN$Once the full verbatim transcript is available via AssemblyAI, a summary layer adds no value and introduces extraction risk. Full transcript -> Claude API extraction -> Supabase operator record is the correct flow.$RSN$,
 'REJECTED', false, null),

-- [2026-06-26] Two-Layer Capture Architecture — CONFIRMED
('2026-06-26', 'sales',
 'Two-Layer Capture Architecture — CONFIRMED',
 $DEC$Two-layer capture architecture confirmed for Track B (CONFIRMED — LOCKED).$DEC$,
 $RSN$Layer 1 (pre-call enrichment) captures verifiable behavioral residue before the call. Layer 2 (post-call transcript extraction) uses the full verbatim transcript -> Claude API extraction -> Supabase operator record, confidence-weighted. Cleanly separates enrichment from extraction.$RSN$,
 'CONFIRMED', false, null),

-- [2026-06-26] Capital Gate — REDEFINED
('2026-06-26', 'sales',
 'Capital Gate — REDEFINED',
 $DEC$Capital gate redefined. The lender performs capital adequacy underwriting (~100% of leads finance via a 60-month term, ~$4K/month). The gate collapses to a binary post-financing field (approved / declined / amount).$DEC$,
 $RSN$Capital adequacy is not an intro-call scoring variable — it is underwritten by the lender post-call. The gate is binary and does not affect the operator score rubric or intro-call architecture.$RSN$,
 'ADOPTED', false, null),

-- [2026-06-26] Bilateral Qualification + Operator-Underwriting Model — ADOPTED
('2026-06-26', 'sales',
 'Bilateral Qualification + Operator-Underwriting Model — ADOPTED',
 $DEC$The Sales Platform is re-scoped from a one-directional persuasion engine to a bilateral qualification system. The introductory call is an underwriting event, not only a sales event. Each primary deal must clear TWO independent scores before it qualifies: (1) Territory score (market quality — exists today via the addressable-market formula) and (2) Operator score (operator quality — NEW). Either score weak = pass and protect the territory. Operator score underwriting factors (captured at intro call): conversion capability, motivation source (grow vs be-rescued), network-additivity, coachability / system-fit, capital + operational seriousness.$DEC$,
 $RSN$Each primary deal permanently encumbers a $179K protected geographic territory and removes it from sellable inventory (no spokes without hub consent). A weak hub therefore sterilizes an entire territory and converts it from asset to liability. A bad "yes" is worse than a "no." The addressable-market formula measures whether a territory is good but is structurally blind to whether the operator can capture it — the single largest risk in the model; the operator score closes that gap. Legal flag: operator-score criteria must be objective, documented, and uniformly applied to avoid discrimination / fair-dealing exposure in a healthcare-network selection context — Rick Dahlson (Jackson Walker) to confirm (flag, not blocker). NOTE: the raw factor rubric is under active redesign (Handoff v2.13 revised 3-factor + capital-gate structure) and is superseded by the Operator Score Architecture entry.$RSN$,
 'ADOPTED', true, null),

-- [2026-06-26] Recording Consent — Blocker Struck (Open Item #8)
('2026-06-26', 'sales',
 'Recording Consent — Blocker Struck (Open Item #8)',
 $DEC$ByrdAdatto call-recording consent review (Open Item #8) removed as a hard build blocker. Replaced with a standing operational rule: all parties must be notified that the meeting is being recorded. Open Item #8 closed.$DEC$,
