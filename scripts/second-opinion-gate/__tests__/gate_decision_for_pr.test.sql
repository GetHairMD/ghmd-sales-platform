-- Second-Opinion Gate — SQL function-level tests for public.gate_decision_for_pr().
-- Closes decision_log row #24. Verifies the three isolation/behaviour invariants:
--   1. returns the single bound row's minimal fields (id, residual_risk, status)
--   2. returns nothing when no row is bound to the PR
...
-- Fixture: one accepted-risk row bound to a sentinel PR number.
insert into ops.decision_log
  (decided_on, platform, title, decision, status, residual_risk, related_pr)
values
  ('2026-07-02', 'sales', '__gate_test_row__', 'test decision', 'ADOPTED', 'accepted', 999999);
...
declare
  ...
  n         integer;
begin
  -- 1. matching row returns exactly one row with the minimal fields.
  select count(*) into n from public.gate_decision_for_pr(999999);
  if n <> 1 then
    raise exception 'TEST 1 FAILED: expected 1 row for PR 999999, got %', n;
  end if;
  select * into r from public.gate_decision_for_pr(999999);
  ...
  select count(*) into n from public.gate_decision_for_pr(888888);
  if n <> 0 then
  ...
  if not has_function_privilege('anon', 'public.gate_decision_for_pr(integer)', 'EXECUTE') then
    raise exception 'TEST 3 FAILED: anon cannot EXECUTE gate_decision_for_pr (CI would fail closed)';
  end if;

  raise notice 'gate_decision_for_pr: all 3 SQL function tests passed.';
end $$;
